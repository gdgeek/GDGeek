	...
	public class VoxelGroups  {
		...
		public class Triangle{
			...
			public List<int> getIndex(){
				...
				Vector3Int n = new Vector3Int(Mathf.RoundToInt(normal.x), Mathf.RoundToInt(normal.y), Mathf.RoundToInt(normal.z));
				if (n == a_.vertice.normal) {
		...
		private Dictionary<Vector3, Dictionary<Color32, Dictionary<Vector3, Point>>> mesh_ = null;// = new Dictionary<Vector3, Point>();
		...
		public static void Mesh2Draw(Dictionary<Vector3, Dictionary<Color32, Dictionary<Vector3, Point>>> mesh, VoxelDrawData draw){
			...
			for (int i = 0; i < nKeys.Count; ++i) {
				Dictionary<Color32, Dictionary<Vector3, Point>> colors = mesh [nKeys [i]];

				List<Color32> cKeys = new List<Color32> (colors.Keys);
		...
		public static Dictionary<Vector3, Dictionary<Color32, Dictionary<Vector3, Point>>>  Draw2Mesh (VoxelDrawData draw){

			Dictionary<Vector3, Dictionary<Color32, Dictionary<Vector3, Point>>> mesh = new Dictionary<Vector3, Dictionary<Color32, Dictionary<Vector3, Point>>>();// = new Dictionary<Vector3, Point>();

			List<VoxelVertex> vex = draw.vertices;

			for (int i = 0; i < vex.Count; ++i) {
				VoxelVertex v = vex[i];
				if (!mesh.ContainsKey (v.normal)) {
					mesh [v.normal] = new Dictionary<Color32, Dictionary<Vector3, Point>> ();
				}
			...
			for (int i = 0; i < trx.Count; i += 3) {
				VoxelVertex a = vex [trx [i]];
				VoxelVertex b = vex [trx [i+1]];
				VoxelVertex c = vex [trx [i+2]];
