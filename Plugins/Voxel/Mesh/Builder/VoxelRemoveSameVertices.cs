...
namespace GDGeek
{
	public class VoxelRemoveSameVertices
	{
		...
		private struct Pack
		{
			...
			public Pack(int i, VoxelVertex v){
				index = i;
				vertice = v;
			}
		...
		public void build(VoxelProduct.Product main){
			...
			List<VoxelVertex> vertices = draw.vertices;
			List<int> triangles = draw.triangles;
			List<VoxelVertex> tVertices = new List<VoxelVertex>();
			...
			while (all.Count != 0) {
				for (int i = 0; i < all.Count; ++i) {
					if (board.ContainsKey (all [i].vertice.position)) {
						...
						if (!ver.vertice.color.Equals(all [i].vertice.color ) || ver.vertice.normal != all [i].vertice.normal) {
							temp.Add (all [i]);
						} else {
