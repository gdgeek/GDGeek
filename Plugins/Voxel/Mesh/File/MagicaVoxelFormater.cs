...
namespace GDGeek{
	public class MagicaVoxelFormater
	{
		...
		public static Color32 Short2Color(ushort s){

			byte r = (byte)(s & 0x1f);
			byte g = (byte)(s >> 5 & 0x1f);
			byte b = (byte)(s >> 10 & 0x1f);
			
			
			return new Color32 (r,g,b, 255);
	
		}
		public static ushort Color2Short(Color32 c){
			ushort s = (ushort)(c.r | c.g<<5 | c.b<<10);
			
		//	s = (ushort)(Mathf.RoundToInt (c.r * 31.0f) | Mathf.RoundToInt (c.g * 31.0f)<<5 | Mathf.RoundToInt (c.b * 31.0f)<<10);
			return s;
		}

		public static Color32 Bytes2Color(VectorInt4 v){
/*
			Color c = new Color ();
		...
		public static VectorInt4 Color2Bytes(Color32 c){
			VectorInt4 v;
			v.x = c.r;
			v.y = c.g;
			v.z = c.b;
		...
		public static MagicaVoxel ReadFromFile(TextAsset file)
			=> ReadFormStream(new MemoryStream(file.bytes));


		public static MagicaVoxel ReadFormStream(Stream stream)
		{
			return ReadFromBinary(new System.IO.BinaryReader(stream));
		}
