...
public class VoxelFromCode : MonoBehaviour {
	...
	void Start () {
		...
		//分别设置颜色和坐标
		datas [0] = new VoxelData (new Vector3Int(0, 0, 0), Color.white);
		datas [1] = new VoxelData (new Vector3Int(0, 1, 0), Color.green);
		datas [2] = new VoxelData (new Vector3Int(0, -1, 0), Color.yellow);
		datas [3] = new VoxelData (new Vector3Int(1, 0, 0), Color.red);
		datas [4] = new VoxelData (new Vector3Int(-1, 0, 0), Color.blue);
		...
		(new VoxelSplitSmall (new Vector3Int(8, 8, 8))).build (product);
		//构造模型
