...
//		Debug.Log (vs2.datas.Length);
		for (int i = 0; i < magic.vs.datas.Count; ++i) {
			...
			Assert.AreEqual(magic.vs.datas[i].position.x, magic2.vs.datas[i].position.x);
			Assert.AreEqual(magic.vs.datas[i].position.y, magic2.vs.datas[i].position.y);
			Assert.AreEqual(magic.vs.datas[i].position.z, magic2.vs.datas[i].position.z);
		}
